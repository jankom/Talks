import processing.core.*; import processing.net.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class rtwclj extends PApplet {
...
public void draw() 
{
  ...
  if (c.available() > 0) {
    input = c.readString();
    d = PApplet.toInt(split(input, ';')); // Split values into an array
    if (d.length == 5) {
      fill(140);
...
class Explosion
{
  ...
  public void make(int x, int y, int cr, int cg, int cb, int size, int mode)
  {
...
static public void main(String args[]) {   PApplet.main(new String[] { "rtwclj" });}}